...
function AliasModal({ onClose, setAlias, onDone }) {
  ...
  const submit = async () => {
    ...
    const userData = { alias: val.trim(), secKey: secKey(), joinedAt: now() };
  ...
  return (
    <div className="modal-backdrop" onClick={e => e.target === e.currentTarget && onClose()}>
      <div className="modal-sheet">
        ...
        <input placeholder="e.g. ghost_77 or st@rfish" value={val} onChange={e => setVal(e.target.value)} style={{ marginBottom: 12 }} onKeyDown={e => e.key === "Enter" && submit()} />
...
function DeadHourPage({ alias, setAlias, setPage }) {
  ...
  useEffect(() => {
    (async () => {
      const aiToggle = await S.get("admin:aiToggle");
      setAiMode(aiToggle !== false);
      const psychStatus = await S.get("admin:psychOnline");
      setPsychOnline(!!psychStatus);
      // Load memory
      const mem = await S.get("psych:memory:" + (alias?.alias || "anon"));
      if (mem && mem.length > 0) setMessages(mem);
    })();
  }, []);
  ...
  const saveMemory = async (msgs) => {
    await S.set("psych:memory:" + (alias?.alias || "anon"), msgs);
  ...
  const sendMessage = async () => {
    ...
    const userMsg = { id: uid(), role: "user", text: input.trim(), ts: now() };
    const newMsgs = [...messages, userMsg];
    setMessages(newMsgs);
    setInput("");
    if (!psychOnline || aiMode) {
      setThinking(true);
      setTimeout(async () => {
        const aiMsg = { id: uid(), role: "psych", text: getAIResponse(newMsgs), ts: now(), isAI: true };
        const finalMsgs = [...newMsgs, aiMsg];
        setMessages(finalMsgs);
        await saveMemory(finalMsgs);
        setThinking(false);
          ...
          <div style={{ maxHeight: 320, overflowY: "auto", display: "flex", flexDirection: "column", gap: 10, marginBottom: 16, paddingRight: 4 }}>
            ...
            {thinking && (
              <div style={{ display: "flex", justifyContent: "flex-start" }}>
                <div className="chat-bubble psych" style={{ display: "flex", gap: 4, alignItems: "center" }}>
...
function AdminOverview({ posts }) {
  ...
  const stats = [
    ...
    { label: "Active Now", value: "Live", icon: "◈" },
  ];
  return (
    <div>
...
function AdminDeadHour() {
  ...
  const [psychOnline, setPsychOnline] = useState(false);
  useEffect(() => {
    S.get("admin:aiToggle").then(v => setAiOn(v !== false));
    S.get("admin:psychOnline").then(v => setPsychOnline(!!v));
  }, []);
  const toggleAI = async () => { const v = !aiOn; setAiOn(v); await S.set("admin:aiToggle", v); };
  const togglePsych = async () => { const v = !psychOnline; setPsychOnline(v); await S.set("admin:psychOnline", v); };
  return (
    <div>
      <div className="serif" style={{ fontSize: 28, color: "#c9a96e", marginBottom: 20 }}>Dead Hour Controls</div>
      <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
        <div style={{ background: "#0d0d0f", border: "1px solid #1a1a1e", borderRadius: 12, padding: 20 }}>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
            <div>
              <div style={{ fontSize: 15, color: "#e8e4dc", marginBottom: 4 }}>AI Psychologist</div>
              <div style={{ fontSize: 12, color: "#5a5660" }}>When ON, AI handles all support requests silently.</div>
            </div>
            <button onClick={toggleAI} style={{ background: aiOn ? "rgba(123,108,246,0.2)" : "#111", border: `1px solid ${aiOn ? "#7b6cf6" : "#1a1a1e"}`, color: aiOn ? "#7b6cf6" : "#5a5660", borderRadius: 8, padding: "8px 16px", fontSize: 13, cursor: "pointer" }}>
              {aiOn ? "ON" : "OFF"}
            </button>
          </div>
        </div>
        <div style={{ background: "#0d0d0f", border: "1px solid #1a1a1e", borderRadius: 12, padding: 20 }}>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
            <div>
              <div style={{ fontSize: 15, color: "#e8e4dc", marginBottom: 4 }}>Psychologist Online</div>
              <div style={{ fontSize: 12, color: "#5a5660" }}>Mark yourself as available to take over from AI.</div>
            </div>
            <button onClick={togglePsych} style={{ background: psychOnline ? "rgba(78,203,141,0.15)" : "#111", border: `1px solid ${psychOnline ? "#4ecb8d" : "#1a1a1e"}`, color: psychOnline ? "#4ecb8d" : "#5a5660", borderRadius: 8, padding: "8px 16px", fontSize: 13, cursor: "pointer" }}>
              {psychOnline ? "ONLINE" : "OFFLINE"}
            </button>
          </div>
        </div>
        <div style={{ background: "#0d0d0f", border: "1px solid rgba(123,108,246,0.2)", borderRadius: 12, padding: 20 }}>
          <div style={{ fontSize: 13, color: "#7b6cf6", marginBottom: 8 }}>Dead Hour Schedule</div>
          <div style={{ fontSize: 12, color: "#5a5660", lineHeight: 1.8 }}>Session 1: 12:00am – 1:00am (local time)<br />Session 2: 3:00am – 5:00am (local time)</div>
        </div>
      </div>
    </div>
  );
